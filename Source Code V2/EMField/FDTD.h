...
*/
class FDTD:public virtual FdtdMemory, public virtual RadiusIndexCacheUser,virtual public Plugin
{
protected:
...
//enable speed recording---------------
	...
	FILE *filehandleStepTime; //file handle for recording time for each step
//-------------------------------------
public:
	FDTD(void);
	virtual ~FDTD(void);
	static int formBaseDataFileName(char *baseDataFileName, const char *classname, unsigned halfGrid, double R0, unsigned timeOrder, unsigned spaceOrder);
	//properties----------------------------------------
