...
void FieldStatistics::Reset()
{
	...
	_divergE = _divergH = _divergE = _divergH = 0.0;
	_rMaxDivE = _rMaxDivH = _rMaxE = _rMaxH = -1;
	_sumDivgE = _sumDivgH = 0.0;
}
...
int FieldStatistics::WriteDivegenceToFile(int filehandle)
{
	int ret = ERR_OK;
	size_t num;
	double se, sh, s;
...
void FieldStatistics::ShowReport(fnProgressReport reporter)
{
	...
	if(_dvgByRadius != NULL)
	{
		...
		reportProcess(reporter,false,"r\tEnergy\tRate\tIn\tOut\tCircle\tDiv. E\tDiv. H");
		for(int i=0;i<=maxRadius;i++)
		{
			num = pointsAt(i);
...
void FieldStatistics::ShowSummary(fnProgressReport reporter)
{
	if(_dvgByRadius != NULL)
	{
		...
		reportProcess(reporter, false, "Max: E(%g, %d), (%g, %d) H(%g, %d), (%g, %d)",
			_maxFieldE,_rMaxE,_maxDivergenceE,_rMaxDivE,_maxFieldH,_rMaxH,_maxDivergenceH,_rMaxDivH); 
		reportProcess(reporter, false, "Sum: E(%g, %g, %g) H(%g, %g, %g)",  
