...
int task170_makePointTimeFile(TaskFile *taskConfig, const char *dataFolder)
{
	...
	if (ret == ERR_OK)
	{
		...
		if (ret == ERR_OK)
		{
			...
			if (ret == ERR_OK)
			{
				err = sprintf_s(outFileTxt, FILENAME_MAX, "%s_%d_%d_%d.txt", fnameBase, m, n, p);
				if (err == -1)
				{
					ret = ERR_MEM_EINVAL;
				}
				...
				if (ret == ERR_OK)
				{
					...
					if (ret == ERR_OK)
					{
						...
						while (ret == ERR_OK)
						{
							...
							//form data file path
							err = sprintf_s(sourceFile, FILENAME_MAX, "%s_%d.em", fnameBase, t);
							if (err == -1)
							{
								ret = ERR_MEM_EINVAL;
								break;
							...
							if (ret == ERR_OK)
							{
								if (FileExists(sourceFileW))
								{
									size_t size1;
									//read fields from simulation data file
									FieldPoint3D* fields = (FieldPoint3D *)ReadFileIntoMemory(sourceFileW, &size1, &ret);
									if (ret == ERR_OK)
									{
										if (fields == NULL)
										{
										...
										else
										{
											...
											if (ret == ERR_OK)
											{
												...
												if (ret == ERR_OK)
												{
													...
													{
														//write to text file
														ret = writefile(fhTxt, buff, (unsigned int)strnlen_s(buff,1024));
													}
												}
											}
											FreeMemory(fields);
